    ...
    public ApplicationUser Parse(IPrincipal principal)
    {
        ...
        if (principal is ClaimsPrincipal claims)
        {
            return new ApplicationUser
            {
                ...
                Address = claims.Claims.FirstOrDefault(x => x.Type == "address_street")?.Value ?? "",
                ZipCode = claims.Claims.FirstOrDefault(x => x.Type == "address_zip_code")?.Value ?? "",
                UserName = claims.Claims.FirstOrDefault(x => x.Type == "user_name")?.Value ?? "",
                MidleName = claims.Claims.FirstOrDefault(x => x.Type == "middle_name")?.Value ?? "",
            };
        }
