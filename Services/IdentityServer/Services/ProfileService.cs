...
namespace BookOnline.IdentityServer.Services
{
    public class ProfileService : IProfileService
    {
        ...
        private async Task<IEnumerable<Claim>> GetClaimsFromUser(ApplicationUser user)
        {
            ...
            if (!string.IsNullOrWhiteSpace(user.MidleName))
                claims.Add(new Claim("midle_name", user.MidleName));
            ...
            if (!string.IsNullOrWhiteSpace(user.Address))
                claims.Add(new Claim("address_street", user.Address));

            if (!string.IsNullOrWhiteSpace(user.ZipCode))
                claims.Add(new Claim("address_zip_code", user.ZipCode));

            if (!string.IsNullOrWhiteSpace(user.PostalCode))
                claims.Add(new Claim("address_postal_code", user.PostalCode));
